    ...
    <>
      {show && (
        <div className="modal-overlay fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
          <div className="modal-content bg-white p-10 rounded-lg shadow-lg w-1/3 h-auto relative">
            ...
            <input
              ...
              className="search-bar w-full p-3 mb-4 border border-grey-300 rounded-md focus:outline-none focus:ring focus:border-green-500"
            />
            {!searchTerm && (
                ...
                <div className="popular-disease-buttons flex flex-wrap gap-2">
                  {popularDiseases.map((disease, index) => (
                    <button
                      ...
                      className="popular-disease-button h-8 px-3 border border-gray-300 rounded-md text-sm font-medium text-gray-600 hover:bg-green-100 hover:text-black hover:border-green-500 focus:outline-none focus:ring focus:border-green-500 transition duration-300"
                    >
                      {disease}
                    </button>
                  ))}
              ...
              <div className="disease-list max-h-40 overflow-auto">
                {filteredDiseases.map((disease, index) => (
                  <div
                    key={index}
                    onClick={() => handleDiseaseClick(disease)}
                    className="other-disease-item py-1 border-b border-gray-300 cursor-pointer text-sm text-gray-600 hover:bg-green-100 hover:text-black hover:border-green-500 transition duration-300"
                  >
                    {disease}
                  </div>
                ))}
